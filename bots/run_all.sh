...
# ORBITAL_BOT_NAME so the dashboard shows one row per strategy. Press
# Ctrl+C once to stop all of them.
#
# Usage:
#   export ORBITAL_API_KEY=sk_live_<your-32-hex-key>          # makers
#   export ORBITAL_TAKER_API_KEY=sk_live_<other-account-key>  # aggressors (optional)
#   ./run_all.sh
#
...
# second Clerk account) and export it as ORBITAL_TAKER_API_KEY. The aggressor
# bots will use it; makers stay on ORBITAL_API_KEY. If TAKER_API_KEY is unset,
# the aggressors fall back to ORBITAL_API_KEY (and STP will keep nuking fills).
#
# Optional overrides:
#   ORBITAL_WS=ws://otherhost:9090/ ./run_all.sh
...
if [ -z "${ORBITAL_API_KEY:-}" ]; then
    echo "warning: ORBITAL_API_KEY not set; bots will use the test placeholder" >&2
    echo "         (set it in your shell or in bots/.env — see .env.example)" >&2
fi
if [ -z "${ORBITAL_TAKER_API_KEY:-}" ]; then
    echo "warning: ORBITAL_TAKER_API_KEY not set; aggressors will share the maker" >&2
...
ORBITAL_BOT_NAME=taker      uv run taker.py         & pids+=($!)
# ext-mm covers all three symbols at ±4bps (tighter than the engine MM's
# ±10bps). It's the user-side "make money" bot — provided ORBITAL_TAKER_API_KEY
# is set so the aggressors below run as a different user_id.
ORBITAL_BOT_NAME=ext-mm     uv run market_maker.py  & pids+=($!)
ORBITAL_BOT_NAME=momentum   uv run momentum.py      & pids+=($!)
ORBITAL_BOT_NAME=mean-rev   uv run mean_reverter.py & pids+=($!)
ORBITAL_BOT_NAME=random     uv run random_walker.py & pids+=($!)
